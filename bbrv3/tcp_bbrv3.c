...
BTF_KFUNCS_START(tcp_bbrv3_check_kfunc_ids)
...
BTF_KFUNCS_END(tcp_bbrv3_check_kfunc_ids)

static const struct btf_kfunc_id_set tcp_bbrv3_kfunc_set = {
	.owner = THIS_MODULE,
	.set   = &tcp_bbrv3_check_kfunc_ids,
};

static int __init bbrv3_register(void)
{
	int ret;

	BUILD_BUG_ON(sizeof(struct bbr) > ICSK_CA_PRIV_SIZE);

	ret = register_btf_kfunc_id_set(BPF_PROG_TYPE_STRUCT_OPS, &tcp_bbrv3_kfunc_set);
	if (ret < 0)
		return ret;
	return tcp_register_congestion_control(&tcp_bbrv3_cong_ops);
}

static void __exit bbrv3_unregister(void)
{
	tcp_unregister_congestion_control(&tcp_bbrv3_cong_ops);
}

module_init(bbrv3_register);
module_exit(bbrv3_unregister);
...
MODULE_ALIAS("tcp-bbrv3");
MODULE_ALIAS("tcp_bbrv3");
